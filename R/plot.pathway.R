...
#'
#' @param data A summary statistics table (data.frame) from limma or DEseq2, where rownames are gene id.
...
plot.pathway <- function(data = df,
        ...
        cal.pathway.scores <- function(data, OUT.Directional = NULL , IS.list = FALSE){
            #supress warnings
            options(warn=-1)
            suppressWarnings({
            suppressMessages({
            ...
            collection_0 <- dlPathwaysDB(pathway.db = pathway.db)
        ...
        if(inherits(data, "list")){
                ...
                path.res <- cal.pathway.scores(data)
